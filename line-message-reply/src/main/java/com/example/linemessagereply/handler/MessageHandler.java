...
import com.example.linemessagereply.Service.WaterQualityService;
		...
		public void text(String replyToken, String text) {
			...
			*/
			if(text.contains("水質")){
				...
				LineConnector.getInstance().replyMessage(LINE_TOKEN, new ReplyMessage(replyToken, new BaseMessage[]{new TextMessage(WaterQualityService.getWaterQualities())}));
			}
			else {
